...
internal class Program
{
    ...
    {
        var ublSettings = new UblXsdSettings
        {
        ...
        var schemaSet = new XmlSchemaSet(new NameTable());
        ...
        {
            var cbcSchema = schemaSet.Schemas(Constants.CommonBasicComponentsTargetNamespace)
                                     .Cast<XmlSchema>()
                                     .Single();
        ...
        schemaSet.MaindocSchemas()
                 .Where(s => s != abstractMaindocBaseSchema)
                 .ToList()
                 .ForEach(s => schemaSet.Reprocess(s));
        schemaSet.Compile();

        var gen = new UblCodeGenerator(ublSettings);
        ...
        XsdTimeTool.IgnoreDateTimeSerializeAsString(allCodeDecls.Where(c => c.Name == "TimeType")
                                                                .Single());

        var mainDocCodeDecls = allCodeDecls.Where(c => c.BaseTypes.Count == 1
                                                       && c.BaseTypes[0]
                                                           .BaseType
                                                       == Constants.abstractBaseSchemaComplexTypeName);

...
public static class XmlSchemaExtensions
{
    ...
    {
        var xsdFilename = new Uri(@this.SourceUri).LocalPath;
    ...
    /// <returns></returns>
    public static string GetCSharpFilename(this XmlSchema @this, string outputDir) => Path.Combine(Path.Combine(outputDir, Path.ChangeExtension(GetFileNameWithSubDirectory(@this), ".cs")));
}
...
public class UblImplicitAssignmentTool
{
    ...
    /// </summary>
    private const string implicitAssignCodeStringFormat =
    ...
    {
        var classes = codeDecls.Where(c => c.IsClass)
                               .ToList();
        ...
        var codeDeclsBase = classes.Where(c =>
                                              ...
                                              && c.Members.OfType<CodeTypeMember>()
                                                  .Any(m =>
                                                           (m.GetType() == typeof(CodeMemberField) || m.GetType() == typeof(CodeMemberProperty)) && (m.Attributes & MemberAttributes.Public) == MemberAttributes.Public && m.HasAnyXmlTextAttribute())
                                   )
                                   .ToList();
        ...
        do
        {
            var baseTypeNameFilter = decendantsAtNextLevel.Select(d => d.CodeDecl.Name)
                                                          .ToArray();
            decendantsAtNextLevel = classes
                                    .Where(c =>
                                               c.BaseTypes.Count > 0
                                               && baseTypeNameFilter.Contains(c.BaseTypes[0]
                                                                               .BaseType)
                                               && !c.HasAnyRequiredMembers()
                                    )
                                    .Select(c => new MemberTypeTuple
                                    {
                                        BaseMemberType = decendantsAtNextLevel.Where(d => d.CodeDecl.Name
                                                                                          == c.BaseTypes[0]
                                                                                              .BaseType)
                                                                              .Select(d => d.BaseMemberType)
                                                                              .Single(),
                                        CodeDecl = c
                                    })
                                    .ToList();
        ...
        foreach (var lowLevelAbstractTuple in accumulatedTupleList.Where(t => t.CodeDecl.GetQualifiedName()
                                                                               .Namespace
                                                                              == Constants.CoreComponentTypeSchemaModuleTargetNamespace)
                                                                  .ToList())
        {
    ...
    {
        var nullTest = parameterType == typeof(string)
            ? "string.IsNullOrEmpty(value) ? null : "
            : "";
        var snipCodeString = string.Format(implicitAssignCodeStringFormat, nullTest, codeDecl.Name, parameterType.FullName, propName);

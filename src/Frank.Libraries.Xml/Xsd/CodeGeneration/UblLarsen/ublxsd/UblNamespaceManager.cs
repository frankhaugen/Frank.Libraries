...
/// <summary>
///     Change to: Give a list of possible cs filesnamespaces to generate. The ones that dont have any codeDelcs just save
///     empty files.
...
/// </summary>
internal class UblNamespaceManager
{
    private static readonly string[] unwantedPrefixes = { "", "xsd", "abs", "cct" }; //,"ccts-cct" ,"ds", "xades" };
...
------------------------------------------------------------------------------";
    ...
    private readonly Dictionary<string, string[]> codeNamespaceUsings;

    // Hardcoded C# using statement resolver 
    private readonly Dictionary<string, string[]> codeNamespaceUsingsNonOptimized = new()
    {
        [""] = new[] { "Cbc", "Cac", "Ext" },
        ["Cbc"] = new[] { "Udt" },
        ["Cac"] = new[] { "Udt", "Qdt", "Cbc" },
    ...
    private readonly Dictionary<string, string[]> codeNamespaceUsingsOptimized = new()
    {
        [""] = new[] { "Cac", "Udt" },
        ["Cac"] = new[] { "Udt" },
        ["Ext"] = new[] { "Udt" },
    ...
    private readonly bool OptionOptimizeCommonBasicComponents;
    private readonly XmlSchemaSet schemaSet;

    private readonly Dictionary<string, string> xml2CSharpNamespaceDictionary;
    ...
    public UblNamespaceManager(XmlSchemaSet schemaSet, string csDefaultNamespace, bool optimizeCommonBasicComponents)
    {
        ...
        OptionOptimizeCommonBasicComponents = optimizeCommonBasicComponents;
        ...
        xml2CSharpNamespaceDictionary = schemaSet.Schemas()
                                                 .Cast<XmlSchema>()
                                                 .SelectMany(schema => schema.Namespaces.ToArray()
                                                                             .Where(qname => !unwantedPrefixes.Contains(qname.Name)))
        ...
        foreach (var schema in schemaSet.MaindocSchemas())
        {
        ...
        if (OptionOptimizeCommonBasicComponents)
        {
        ...
        codeNamespaceUsings = codeNamespaceUsings.ToDictionary(k => csDefaultNamespace
                                                                    + (k.Key == ""
                                                                        ? ""
                                                                        : ".")
                                                                    + k.Key, v => v.Value);
    ...
    ///     Some schemas will not contain any CodeTypeDeclaration/generated code, and hence, c# file will be empty with a
    ///     header only.
    /// </summary>
    public IEnumerable<XmlSchema> Schemas => schemaSet.Schemas()
                                                      .Cast<XmlSchema>();
    ...
    public CodeNamespace GetCodeNamespaceForXmltargetNamespace(string xmlNamespace)
    {
        ...
        {
            var csScopeName = xml2CSharpNamespaceDictionary[xmlNamespace];
            var codeNs = new CodeNamespace(csScopeName);
            var commentLine = GetCommentTextForScope(csScopeName);
            var headerComment = new CodeComment(string.Format(nsHeaderComment, commentLine));
            var fileHeader = new CodeCommentStatement(headerComment);
            ...
            {
                foreach (var usingNamespace in codeNamespaceUsings[csScopeName])
                {
    ...
    private string GetCommentTextForScope(string csScopeName)
    {
        ...
        if (res != string.Empty)
        {
            res = Environment.NewLine + res + Environment.NewLine;
        }

...
{
    private static readonly Dictionary<string, XmlSchemaDocumentation> emptyDocumentationDictionary = new();
    ...
    public static void AddDocumentation(IEnumerable<CodeTypeDeclaration> codeDecls)
    {
        Func<CodeTypeDeclaration, IQueryable<CodeTypeMember>> GetPublicFieldPropertyMembers = cdecl => cdecl.Members.OfType<CodeTypeMember>()
                                                                                                            .Where(c => (c.GetType() == typeof(CodeMemberField) || c.GetType() == typeof(CodeMemberProperty)) && (c.Attributes & MemberAttributes.Public) == MemberAttributes.Public)
                                                                                                            .AsQueryable();
        ...
        foreach (var codeDecl in codeDecls)
        {
            var xsdComplexType = (XmlSchemaComplexType)codeDecl.GetXmlSchemaType();
            ...
            var xsdDocNode = xsdComplexType.Annotation?.Items.OfType<XmlSchemaDocumentation>()
                                           .FirstOrDefault();
            if (xsdDocNode != null)
            {
            ...
            var memberDocumentationDictionary = emptyDocumentationDictionary;
            switch (xsdComplexType.ContentType)
            {
                case XmlSchemaContentType.TextOnly: // simpleContent, DateType
                    var sext = (xsdComplexType.ContentModel as XmlSchemaSimpleContent).Content as XmlSchemaSimpleContentExtension;
                    memberDocumentationDictionary = sext.Attributes.OfType<XmlSchemaAttribute>()
                                                        .ToDictionary(k => k.Name, v => v.Annotation?.Items.OfType<XmlSchemaDocumentation>()
                                                                                         .First());
                    break;
                ...
                case XmlSchemaContentType.Mixed:
                    if (xsdComplexType.ContentTypeParticle is XmlSchemaSequence)
                    {
                        ...
                        memberDocumentationDictionary = schemaSequence.Items.OfType<XmlSchemaElement>()
                                                                      .ToDictionary(k => k.QualifiedName.Name, v => v.Annotation?.Items.OfType<XmlSchemaDocumentation>()
                                                                                                                     .FirstOrDefault());
                    }
                    else if (xsdComplexType.ContentTypeParticle is XmlSchemaChoice)
                    {
                        ...
                        memberDocumentationDictionary = schemaChoice.Items.OfType<XmlSchemaElement>()
                                                                    .ToDictionary(k => k.QualifiedName.Name, v => v.Annotation?.Items.OfType<XmlSchemaDocumentation>()
                                                                                                                   .FirstOrDefault());
                    }

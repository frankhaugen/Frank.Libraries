...
internal class UblSchemaTypeSimplificationTool
{
    ...
    public static void SimplifyCommonBasicComponentsTypes(XmlSchema schema)
    {
        ...
        var repaceElements = schema.Items.OfType<XmlSchemaElement>()
                                   .Select(e => new XmlSchemaElement { Name = e.Name, SchemaTypeName = typeLookup[e.SchemaTypeName] })
                                   .ToList();
        schema.Items.Clear();
    ...
    {
        var ccts_cctPrefix = "cctscct";
        var coreCompSchema = schemaSet.Schemas(Constants.CoreComponentTypeSchemaModuleTargetNamespace)
                                      .OfType<XmlSchema>()
                                      .Single();
        var unqualSchema = schemaSet.Schemas(Constants.UnqualifiedDataTypesTargetNamespace)
                                    .OfType<XmlSchema>()
                                    .Single();

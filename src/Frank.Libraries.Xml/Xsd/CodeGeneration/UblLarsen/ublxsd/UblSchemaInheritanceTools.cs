...
public class UblSchemaInheritanceTools
{
    ...
    public static XmlSchema ModifyMaindocSchemasForInheritance(ICollection<XmlSchema> maindocSchemas)
    {
        ...
        var abstactBaseSchemaQNameToInheritFrom = new XmlQualifiedName(Constants.abstractBaseSchemaComplexTypeName, abstractBaseSchema.TargetNamespace);
        ...
        foreach (var maindocSchema in maindocSchemas)
        {
            ...
            var maindocSchemaComplexType = maindocSchema.Items.OfType<XmlSchemaComplexType>()
                                                        .Single(); // Single is safe. Should only be one.
    ...
    private static XmlSchema CreateAbstractBaseSchemaFromMaindocSchema(XmlSchema templateSchema, int sharedElementCount)
    {
        ...
        var abstractBaseElement = abstractBaseSchema.Items.OfType<XmlSchemaElement>()
                                                    .Single(); // Single. There can only be one
        var abstractBaseComplexType = abstractBaseSchema.Items.OfType<XmlSchemaComplexType>()
                                                        .Single(); // Christopher Lambert again
        ...
        doc.Markup = new XmlNode[]
        {
            nodeCreaterDoc.CreateTextNode("This is a custom generated class that holds all the props/fields common to all UBL maindocs."),
        ...
        var elementCollection = (abstractBaseComplexType.Particle as XmlSchemaSequence).Items;
    ...
    private static int GetSharedElementCount(ICollection<XmlSchema> maindocSchemas) =>
        maindocSchemas
            .Select(s => s.Items.OfType<XmlSchemaComplexType>()
                          .Single())
            .Select(c => (c.ContentTypeParticle as XmlSchemaSequence).Items.Cast<XmlSchemaElement>()
                                                                     .ToArray())
            .OrderBy(c => c.Length)
            .Aggregate((acc, next) => acc.AsEnumerable()
                                         .TakeWhile((s, i) => s.QualifiedName
                                                              == next[i]
                                                                  .QualifiedName)
                                         .ToArray())
            .Count();

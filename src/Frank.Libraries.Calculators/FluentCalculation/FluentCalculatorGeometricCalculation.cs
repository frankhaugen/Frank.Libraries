...
namespace Frank.Libraries.Calculators.FluentCalculation
{
    public static class FluentCalculatorGeometricCalculation
    {
        ...
        {
            var x = origin.X.ToDecimal().Subtract(destination.X.ToDecimal()).Pow(2);
            var y = origin.Y.ToDecimal().Subtract(destination.Y.ToDecimal()).Pow(2);
        ...
        {
            var x = origin.X.Subtract(destination.X).ToDecimal().Pow(2);
            var y = origin.Y.Subtract(destination.Y).ToDecimal().Pow(2);
            var z = origin.Z.Subtract(destination.Z).ToDecimal().Pow(2);
            var result = x.Add(y).Add(z).Sqrt();
            return result.ToFloat();
        }

        public static decimal Distance(this CartesianCoordinates origin, CartesianCoordinates destination)
        {
            var x = origin.X.Subtract(destination.X).Pow(2);
            var y = origin.Y.Subtract(destination.Y).Pow(2);
            var z = origin.Z.Subtract(destination.Z).Pow(2);
            var result = x.Add(y).Add(z).Sqrt();
            return result;
        }
        ...
        {
            var x = origin.X.ToDecimal().Subtract(destination.X).Pow(2);
            var y = origin.Y.ToDecimal().Subtract(destination.Y).Pow(2);
            var z = origin.Z.ToDecimal().Subtract(destination.Z).Pow(2);
            var result = x.Add(y).Add(z).Sqrt();
            return result.ToFloat();
        }

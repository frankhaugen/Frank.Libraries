...
{
    public static string ToJson<T>(this T source, bool indented = true) where T : class => JsonSerializer.Serialize(source, new JsonSerializerOptions { WriteIndented = indented });

    public static T FromJson<T>(this string source) where T : class => JsonSerializer.Deserialize<T>(source)!;
    ...
    public static string SerializeObjectToJson<T>(this T value) where T : class, new() =>
        ...
        JsonSerializer.Serialize(value, new JsonSerializerOptions { WriteIndented = true }); // return the value..
}
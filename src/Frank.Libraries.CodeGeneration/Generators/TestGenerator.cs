...
{
    public string Generate(string namespaceName, string className) => Generate(namespaceName, GetType()
                                                                                              .Assembly.GetTypes()
                                                                                              .First(x => x.Name == className && x.Namespace == namespaceName));
    ...
    public string Generate(string namespaceName, Type type, List<string>? ignoreMethods = null)
    {
        ...
        var returnTypes = type.GetMethods()
        ...
        constructorParameters.DoForEach(x => namespaces.AddUnique(x.GetType()
                                                                   .Namespace));
        types.Distinct(x => x.Name)
             .DoForEach(x => namespaces.AddUnique(x.Namespace));
        ...
        writer.WithCurlyBraces($"public class {type.Name}Tests", () =>
        {
            ...
            if (isNotStatic)
            {
                ...
                var methodLookup = type.GetMethods()
                                       .ToLookup(x => x.Name, info => info);

                foreach (var methodGrouping in methodLookup)
                {
                    if (ignoreMethods != null && ignoreMethods.Any(x => x == methodGrouping.Key))
                    {
                        continue;
                    }
            ...
            else
            {
                ...
                foreach (var methodGrouping in methodLookup)
                {
                    ...
                    if (methodGrouping.Count() == 1)
                    {
                        ...
                        if (methodGrouping.First()
                                          .IsDefined(typeof(ExtensionAttribute)))
                        {
                            SetupExtensionMethodTest(writer, methodGrouping.First());
                        }
                    }
                    else
                    {
                        ...
                        foreach (var methodInfo in methodGrouping)
                        {
                            ...
                            {
                                SetupExtensionMethodTest(writer, methodInfo, i.ToString());
                            }
                        }
                    }
                }
    ...
    private void SetupExtensionMethodTest(CodegenTextWriter writer, MethodInfo method, string suffix = "")
    {
        ...
        writer.WithCurlyBraces($"public {returnType} {method.Name}Test{suffix}()", () =>
        {

...
namespace Frank.Libraries.Time
{
    public class DateTimeService
    {
        ...
        public int DayOfWeek => (int)_clock.GetCurrentInstant().InUtc().DayOfWeek;

        public Instant Instant() => _clock.GetCurrentInstant();
        public DateTime DateTime() => _clock.GetCurrentInstant().ToDateTimeUtc();
        public DateTimeOffset DateTimeOffset() => _clock.GetCurrentInstant().ToDateTimeOffset();
        public TimeSpan TimeOfDay() => _clock.GetCurrentInstant().ToDateTimeUtc().TimeOfDay;
        public ZonedDateTime ZonedDateTime(string ianaTimezone) => _clock.GetCurrentInstant().InZone(DateTimeZoneProviders.Tzdb[ianaTimezone]);
        public ZonedDateTime UtcDateTime() => _clock.GetCurrentInstant().InUtc();

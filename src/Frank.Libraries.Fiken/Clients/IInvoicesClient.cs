...
public interface IInvoicesClient
{
    ...
    ///     The invoiceId (primary key of the returned object) is returned in the GET all invoices call;
    ///     not to be confused with invoiceNumber
    /// </param>
    ...
    /// <summary>
    ///     Creates an invoice. There are two types of invoice lines that can be added to an invoice line: product line or free
    ///     text line. Provide a product Id if you are invoicing a product. All information regarding the price and VAT for
    ///     this product will be added to the invoice. It is however also possible to override the unit amount by sending
    ///     information in both fields \&quot;productId\&quot; and \&quot;unitAmount\&quot;. An invoice line can also be a free
    ///     text line meaning that no existing product will be associated with the invoiced line. In this case all information
    ...
    /// <summary>
    ///     Returns all invoices for given company. You can filter based on issue date, last modified date, customer ID, and if
    ///     the invoice is settled or not.
    /// </summary>
    /// <param name="companySlug">Slug of company to retrieve</param>
    /// <param name="page">
    ///     Returns the number of the page to return. Valid page values are integers from 0 to the total number
    ...
    ///     Returns only invoices sent to specified customer. Must be the contactId provided by a GET
    ///     contacts call.
    /// </param>
    /// <param name="settled">
    ///     When set to true, returns only invoices that have been settled. Otherwise false returns all
    ///     invoices that have not been fully settled.
    /// </param>
    /// <param name="orderReference">Filter based on order reference for a given invoice</param>
    ...
    /// <summary>
    ///     Updates invoice with provided id. It is possible to update the due date of an invoice as well as if the invoice was

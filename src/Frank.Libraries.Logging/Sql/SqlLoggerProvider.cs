...
public sealed class SqlLoggerProvider : ILoggerProvider
{
    ...
    public SqlLoggerProvider(SqlLoggerConfiguration configuration) => _configuration = configuration;
    ...
    public ILogger CreateLogger(string categoryName)
    {
        if (TableExists())
        {
            return _loggers.GetOrAdd(categoryName, name => new SqlLogger(name, _configuration));
        }

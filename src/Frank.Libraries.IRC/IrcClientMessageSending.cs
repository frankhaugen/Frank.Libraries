...
partial class IrcClient
{
    ...
    /// <summary>
    ///     Sends the password for registering the connection. This message must only be sent before the actual registration,
    ///     which is done by <see cref="SendMessageUser" /> (for normal users) or <see cref="SendMessageService" /> (for
    ///     services).
    /// </summary>
    /// <param name="password">The connection password.</param>
    protected void SendMessagePassword(string password) => WriteMessage(null, "pass", password);

    /// <summary>
    ///     Sends the nick name of the local user to the server. This command may be used either for intitially setting the
    ///     nick name or changing it at any point.
    /// </summary>
    /// <param name="nickName">The nick name to set.</param>
    protected void SendMessageNick(string nickName) => WriteMessage(null, "nick", nickName);
    ...
    protected void SendMessageUser(string userName, int userMode, string realName) => WriteMessage(null, "user", userName, userMode.ToString(), "*", realName);
    ...
    ///     A wildcard expression for matching against server names, which determines where the service
    ///     is visible.
    /// </param>
    /// <param name="description">A description of the service.</param>
    protected void SendMessageService(string nickName, string distribution, string description = "") => WriteMessage(null, "service", nickName, distribution, "0", "0", description);
    ...
    protected void SendMessageOper(string userName, string password) => WriteMessage(null, "oper", userName, password);
    ...
    protected void SendMessageUserMode(string nickName, string modes = null) => WriteMessage(null, "mode", nickName, modes);
    ...
    protected void SendMessageQuit(string comment = null) => WriteMessage(null, "quit", comment);
    ...
    protected void SendMessageSquit(string targetServer, string comment) => WriteMessage(null, "squit", targetServer, comment);

    /// <summary>
    ///     Sends a request to leave all channels in which the user is currently present.
    /// </summary>
    protected void SendMessageLeaveAll() => WriteMessage(null, "join", "0");
    ...
    /// <param name="comment">The comment to send the server, or <see langword="null" /> for none.</param>
    protected void SendMessagePart(IEnumerable<string> channels, string comment = null) => WriteMessage(null, "part", string.Join(",", channels), comment);
    ...
    protected void SendMessageTopic(string channel, string topic = null) => WriteMessage(null, "topic", channel, topic);

    /// <summary>
    ///     Sends a request to list all names visible to the client.
    /// </summary>
    /// <param name="channels">
    ///     A collection of the names of channels for which to list users, or <see langword="null" /> for
    ///     all channels.
    /// </param>
    ...
    /// </summary>
    /// <param name="channels">A collection of the names of channels to list, or <see langword="null" /> for all channels.</param>
    /// <param name="targetServer">
    ///     The name of the server to which to forward the message, or <see langword="null" /> for the
    ///     current server.
    /// </param>
    ...
    protected void SendMessageInvite(string channel, string nickName) => WriteMessage(null, "invite", nickName, channel);
    ...
    protected void SendMessageKick(string channel, IEnumerable<string> nickNames, string comment = null) => WriteMessage(null, "kick", channel, string.Join(",", nickNames), comment);

    /// <summary>
    ///     Sends a request to kick the specifier users from the specified channel.
    /// </summary>
    /// <param name="channelsUsers">
    ///     A collection of 2-tuples of channel names and the nick names of the users to kick from the
    ///     channel.
    /// </param>
    /// <param name="comment">The comment to send the server, or <see langword="null" /> for none.</param>
    ...
    ///     current server.
    /// </param>
    protected void SendMessageMotd(string targetServer = null) => WriteMessage(null, "motd", targetServer);

    /// <summary>
    ///     Sends a request to get statistics about the size of the IRC network.
    /// </summary>
    /// <param name="serverMask">
    ///     A wildcard expression for matching against the names of servers, or <see langword="null" /> to
    ///     match the entire network.
    /// </param>
    /// <param name="targetServer">
    ///     The name of the server to which to forward the message, or <see langword="null" /> for the
    ///     current server.
    /// </param>
    protected void SendMessageLUsers(string serverMask = null, string targetServer = null) => WriteMessage(null, "lusers", serverMask, targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageVersion(string targetServer = null) => WriteMessage(null, "version", targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageStats(string query = null, string targetServer = null) => WriteMessage(null, "stats", query, targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageLinks(string serverMask = null, string targetServer = null) => WriteMessage(null, "links", targetServer, serverMask);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageTime(string targetServer = null) => WriteMessage(null, "time", targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageConnect(string hostName, int port, string targetServer = null) => WriteMessage(null, "connect", hostName, port.ToString(), targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageTrace(string targetServer = null) => WriteMessage(null, "trace", targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageAdmin(string targetServer = null) => WriteMessage(null, "admin", targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessageInfo(string targetServer = null) => WriteMessage(null, "info", targetServer);
    ...
    protected void SendMessageServlist(string mask = null, string type = null) => WriteMessage(null, "servlist", mask, type);
    ...
    /// <param name="text">The text of the message to send.</param>
    protected void SendMessageSquery(string serviceName, string text) => WriteMessage(null, "squery", serviceName, text);
    ...
    /// </summary>
    /// <param name="nickNameMasks">A collection of wildcard expressions for matching against the nick names of users.</param>
    /// <param name="targetServer">
    ///     The name of the server to which to forward the message, or <see langword="null" /> for the
    ///     current server.
    /// </param>
    protected void SendMessageWhoIs(IEnumerable<string> nickNameMasks, string targetServer = null) => WriteMessage(null, "whois", targetServer, string.Join(",", nickNameMasks));

    /// <summary>
    ///     Sends a request to perform a WhoWas query on users.
    /// </summary>
    /// <param name="nickNames">A collection of wildcard expressions for matching against the nick names of users.</param>
    ...
    /// <param name="comment">The comment to send the server.</param>
    protected void SendMessageKill(string nickName, string comment) => WriteMessage(null, "kill", nickName, comment);
    ...
    ///     current server.
    /// </param>
    protected void SendMessagePing(string server, string targetServer = null) => WriteMessage(null, "ping", server, targetServer);
    ...
    ///     current server.
    /// </param>
    protected void SendMessagePong(string server, string targetServer = null) => WriteMessage(null, "pong", server, targetServer);

    /// <summary>
    ///     Sends an update to the server indicating that the local user is away.
    /// </summary>
    /// <param name="text">
    ///     The text of the away message. The away message is sent to any user that tries to contact the local
    ///     user while it is away.
    /// </param>
    protected void SendMessageAway(string text = null) => WriteMessage(null, "away", text);

    /// <summary>
    ///     Sends a request to the server telling it to reprocess its configuration settings.
    /// </summary>
    protected void SendMessageRehash() => WriteMessage(null, "rehash");

    /// <summary>
    ///     Sends a request to the server telling it to shut down.
    /// </summary>
    protected void SendMessageDie() => WriteMessage(null, "die");

    /// <summary>
    ///     Sends a message to the server telling it to restart.
    /// </summary>
    protected void SendMessageRestart() => WriteMessage(null, "restart");
    ...
    ///     current server.
    /// </param>
    protected void SendMessageUsers(string targetServer = null) => WriteMessage(null, "users", targetServer);

    /// <summary>
    ///     Sends a message to all connected users that have the 'w' mode set.
    /// </summary>
    /// <param name="text">The text of the message to send.</param>
    protected void SendMessageWallops(string text) => WriteMessage(null, "wallops", text);
    ...
    protected void SendMessageUserHost(IEnumerable<string> nickNames) => WriteMessage(null, "userhost", nickNames);

    /// <summary>
    ///     Sends a request to check whether the specified users are currently online.
    /// </summary>
    /// <param name="nickNames">A collection of the nick names of the users to query.</param>
    protected void SendMessageIsOn(IEnumerable<string> nickNames) => WriteMessage(null, "ison", nickNames);
}
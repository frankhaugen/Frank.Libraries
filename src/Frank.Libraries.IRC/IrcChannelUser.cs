...
public class IrcChannelUser : INotifyPropertyChanged
{
    ...
    public void Kick(string comment = null) => channel.Kick(User.NickName, comment);

    /// <summary>
    ///     Gives the user operator privileges in the channel.
    /// </summary>
    public void Op() => channel.SetModes("+o", User.NickName);

    /// <summary>
    ///     Removes operator privileges from the user in the channel.
    /// </summary>
    public void DeOp() => channel.SetModes("-o", User.NickName);

    /// <summary>
    ///     Voices the user in the channel.
    /// </summary>
    public void Voice() => channel.SetModes("+v", User.NickName);

    /// <summary>
    ///     Devoices the user in the channel
    /// </summary>
    public void DeVoice() => channel.SetModes("-v", User.NickName);
    ...
    public override string ToString() => string.Format("{0}/{1}", channel.Name, User.NickName);
}
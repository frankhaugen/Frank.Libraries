...
public class IrcLocalUser : IrcUser, IIrcMessageSendHandler, IIrcMessageReceiveHandler, IIrcMessageReceiver
{
    ...
    void IIrcMessageSendHandler.HandleMessageSent(IList<IIrcMessageTarget> targets, string text) => HandleMessageSent(targets, text);

    void IIrcMessageSendHandler.HandleNoticeSent(IList<IIrcMessageTarget> targets, string text) => HandleNoticeSent(targets, text);
    ...
    /// </summary>
    public void UnsetAway() => Client.UnsetAway();

    /// <summary>
    ///     Requests a list of the current modes of the user.
    /// </summary>
    public void GetModes() => Client.GetLocalUserModes(this);

    /// <inheritdoc cref="SetModes(IEnumerable{char})" />
    public void SetModes(params char[] newModes) => SetModes((IEnumerable<char>)newModes);
    ...
    internal void HandleJoinedChannel(IrcChannel channel) => OnJoinedChannel(new IrcChannelEventArgs(channel));

    internal void HandleLeftChannel(IrcChannel channel) => OnLeftChannel(new IrcChannelEventArgs(channel));

    internal void HandleMessageSent(IList<IIrcMessageTarget> targets, string text) => OnMessageSent(new IrcMessageEventArgs(this, targets, text, Client.TextEncoding));

    internal void HandleNoticeSent(IList<IIrcMessageTarget> targets, string text) => OnNoticeSent(new IrcMessageEventArgs(this, targets, text, Client.TextEncoding));

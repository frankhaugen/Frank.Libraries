...
public class CoordinatesConverterTests
{
    ...
    public CoordinatesConverterTests(ITestOutputHelper outputHelper) => _outputHelper = outputHelper;
    ...
    public void CartesianToSpherical_()
    {
        ...
        // Assert
        result2.X.Should()
               .BeApproximately(coordinates.X, 0.05m);
        result2.Y.Should()
               .BeApproximately(coordinates.Y, 0.05m);
        result2.Z.Should()
               .BeApproximately(coordinates.Z, 0.05m);
        result.Inclination.Should()
              .BeApproximately(0.9553166181245092781638571022m, 0.05m);
        result.Azimuth.Should()
              .BeApproximately(0.7853981633974483096156608458m, 0.05m);
        result.Radius.Should()
              .BeApproximately(173.20508075688772935274463415m, 0.05m);
    }
    ...
    public void SphericalToCartesian_()
    {
        ...
        // Assert
        result.X.Should()
              .BeApproximately(100m, 0.05m);
        result.Y.Should()
              .BeApproximately(100m, 0.05m);
        result.Z.Should()
              .BeApproximately(100m, 0.05m);
        result2.Inclination.Should()
               .BeApproximately(polarCoordinates.Inclination, 0.05m);
        result2.Azimuth.Should()
               .BeApproximately(polarCoordinates.Azimuth, 0.05m);
        result2.Radius.Should()
               .BeApproximately(polarCoordinates.Radius, 0.05m);
    }
}
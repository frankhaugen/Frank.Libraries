...
public class JsonContextTests
{
    ...
    private JsonContext<JsonTestModel> Setup(Guid sessionId) => new JsonContext<JsonTestModel>(Options.Create(new JsonConfiguration { Folder = Path.Combine(TestFolder, sessionId.ToString()) }), _mockFileSystem);

    private void Teardown(Guid sessionId)
    {
